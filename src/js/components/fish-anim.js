	...
	function accioPeix() {
		...
		do {
			...
			if (w < 500) {
				...
				x = Math.floor(Math.random() * (w - maxFishX)) + maxFishX / 2;
			}
		} while (x > formRect.left && x < formRect.right && y > formRect.top && y < formRect.bottom);
...
window.addEventListener('DOMContentLoaded', function () {
    ...
    // Функция для изменения z-index элементов .suportPeix
    function changeZIndexOfSuportPeix(zIndexValue) {
        document.querySelectorAll('.suportPeix').forEach(function(suportPeix) {
            suportPeix.style.zIndex = zIndexValue;
        });
    }

    // Обработчики событий для formWrapper
    formWrapper.addEventListener('mouseenter', function() {
        changeZIndexOfSuportPeix('-1');
    });
    ...
    // Добавляем обработчики событий focus, input и mouseenter на каждый элемент ввода
