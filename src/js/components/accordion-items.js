$(document).ready(function() {
    $(".accordion").on("click", function(event) {
        ...
        if(parentPanel.length) {
            parentPanel.css("maxHeight", parentPanel.prop("scrollHeight") + panel.prop("scrollHeight") + paddingOffset + "px");
        }
...
// Устанавливаем начальное состояние для .child-panel, чтобы оно было открыто, исключая .last-panel
    $(".child-panel:not(.last-panel)").css({
        "maxHeight": function() { return $(this).prop("scrollHeight") + 20 + "px"; },
        "padding-top": "10px",
        "padding-bottom": "10px"
    });
